...
**6. Create a scatterplot showing the relationship between the Median household income expressed in 2018 dollars and the full-time median weekly price charged for Center-based childcare for an infant. Overlay a linear regression line (lm) to show the trend.**

```{r}
# plot for scatterplot
```

**7. Look up the documentation for `lm()` to fit a linear regression model (recall: y = mx+b) to the data. Identify the coefficient estimates from the model.**
...
**8. Do you have evidence to conclude there is a relationship between the median household income and the median weekly cost of Center-based childcare for infants in CA? Cite values from your output for support.**
...
**Investigate the full-time median price charged for childcare in a Center-based setting versus the full-time median price charged for childcare in a Family (in-home) setting for California. Pay close attention to the different developmental stages. Support your answer with at least one table of summary statistics and one visualization.**
