...
subtitle: "Tidy Data with tidyr & Factor Ordering in Visualizations with forcats"
format: 
  html:
    self-contained: true
    code-tools: true
...
::: callout-tip
To join by different variables on `datasetX` and `datasetY`, use a named vector. For example, `by = join_by(a == b)` will match `datasetX$a` to `datasetY$b`.
:::
...
The comparison I am interested in looking at is the military expenditures between Countries in Eastern Europe. 
Something like this:
...
::: callout-warning
Unfortunately, if we want a point representing the spending for every country and year, this requires that every year is included in one column!
:::
...
**5. Pivot the cleaned up `military` data set to a "longer" orientation. Save this new "long" version as a new data set called `military_long`.**

::: callout-caution
**Do not** overwrite your cleaned up data set. Think about code organization.
:::
...
**6. Notice that when you pivoted the data set, the `Year` variable is a character data type. Convert this to numeric.**

```{r}
summary(military_long)

# insert code for converting Year to numeric -- make sure to save this change to military long
```

## Data Visualization Exploration

**7. Create a plot to explore the military spending between the Eastern European Countries**

```{r}
# I have provided a list of the countries in Eastern Europe (in this data set) for you to use.
...
**8. Modify your code above to order the countries from largest to smallest *median* military expenditure spending.**

::: callout-tip
Look at the function options from the `forcats` cheatsheet.
:::
