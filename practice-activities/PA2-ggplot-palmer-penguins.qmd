...
::: callout-note
### `install.packages()` in Console NOT `.qmd` file

You should type this in your console and NOT include it in a code chunk in your `.qmd` file. Recall, we only have to *install* a package once, but *load* it each time. Each time to Render your `.qmd` file the code chunks are run, thus it would continue to unnecessarily install the package over and over again.
:::
...
I like to tangibly see the data set I will be working with so I am going to pull the `penguins` data into my R environment (do you see it in the top right Environment tab?).
...
Follow good code format!
:::

7. Use `ggplot2` to draw a barchart of `species`.

```{r}

```

8. Use `ggplot2` to create a scatterplot of the relationship between the bill length (`bill_length_mm`) and bill depth (`bill_depth_mm`).

```{r}

```

9. Building off of (8), add an aesthetic to differentiate the species on your scatterplot by color.

```{r}

```

10. Building off of (9), add the location of the penguins, `island`, to your visualization. There may be more than one method to address this, however, one method will more easily address the questions below.
...
## If you finish early... take everything you know, and use it for evil (up to +6 FP)
...
Once you have created the ugliest graphic you can, [save your plot](https://r4ds.hadley.nz/data-visualize.html#sec-ggsave), and upload it to the [shared google slides](https://docs.google.com/presentation/d/1INpPdMjh7nUqv21OLGMWeAVaAuLL3N-aicmpOfFc30Q/edit#slide=id.g2aab62f17a9_0_0) along with how you modified the chart - let's share code! - your explanation as to why you made the decisions you did, and which principles of good graphics you’ve intentionally violated.

To receive your flex points, upload the url link to your slide to the Canvas Assignment by **Monday, January 22 at 11:59pm**. We want to be able to look at them during class next week!

::: callout-warning
If you are on your slide, you can copy-paste the URL from the top of your browser just as you would any other website. This is NOT the link that you click on to Share.
:::
