...
You will be given a `.qmd` template similar to lab assignments and asked a series of short questions about the minimum wage data set, such as *"Which course(s) had the highest and lowest average rating for instructors with more than 6 years of experience?"* Each of these questions should be answered *exactly* using functions from the `tidyverse`.
